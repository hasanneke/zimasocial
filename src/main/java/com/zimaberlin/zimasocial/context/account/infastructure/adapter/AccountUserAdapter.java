...
public class AccountUserAdapter {
     public Account convertUserEntityToAccount(UserEntity user) {
        ...
        return new Account(new AccountId(user.getId()), user.getEmail(), user.getAuthProvider(), user.getRoles(), user.getIsDisabled(), user.getDisableDate(), user.getDeleteDate(), user.getIsDeleted(), user.isPrivate(), user.getDeleteReason(), user.getDisableReason());
    }
}
